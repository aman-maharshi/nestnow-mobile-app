          ...
          {property.reviews.length > 0 && (
            <View className="mt-7">
              ...
              {property.reviews.map((item: any, index: number) => (
                <View key={index} className="mt-5 p-4 bg-gray-50 rounded-lg">
                  <View className="flex-row items-center mb-2">
                    <Image source={{ uri: item.avatar }} className="w-10 h-10 rounded-full mr-3" />
                    <View className="flex-1">
                      <Text className="text-base font-rubik-bold text-black-300">{item.user}</Text>
                      <Text className="text-sm font-rubik text-black-200">{item.date}</Text>
                    </View>
